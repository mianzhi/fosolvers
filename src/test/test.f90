...
subroutine testList()
  ...
  &  ListDScal(2)%len==6)then
    call showPass('ListDScal push and get')
  else
    call showFail('ListDScal push and get')
  end if
  call ListDScal(1)%clear()
  if(ListDScal(1)%len==0.and..not.allocated(ListDScal(1)%dat))then
    call showPass('ListDScal clear')
  end if
  
  call ListDVect%specDim(3)
  call ListDVect%extend(1)
  call ListDVect%push([1d0,2d0,3d0])
  a(:,1)=[4d0,5d0,6d0]
  a(:,2)=[2d0,3d0,4d0]
  call ListDVect%push(a(:,1:2))
  a(:,:)=ListDVect%get([1,2,3])
  if(abs(a(1,1)-1d0)<TOLERANCE.and.&
  &  abs(a(3,2)-6d0)<TOLERANCE.and.&
  &  abs(a(3,3)-4d0)<TOLERANCE)then
    call showPass('ListDVect push and get')
  else
    call showFail('ListDVect push and get')
  end if
end subroutine
...
subroutine testSerialPack()
  ...
  call dlist%addto(sp)
  call vlist%specDim(3)
  call vlist%extend(1)
  call vlist%push([1d0,2d0,3d0])
  a(:,1)=[4d0,5d0,6d0]
  a(:,2)=[2d0,3d0,4d0]
  call vlist%push(a(:,1:2))
  call vlist%addto(sp)
  ...
  if(all(list(3)%get([(i,i=1,list(3)%len)])==[3,4,5]).and.&
  &  list(1)%len==0.and.&
  &  all(list(2)%get([(i,i=1,list(2)%len)])==[3,4,5]))then
  ...
  call vlist%recover(sp)
  a(:,:)=vlist%get([1,2,3])
  if(abs(a(1,1)-1d0)<TOLERANCE.and.&
  &  abs(a(3,2)-6d0)<TOLERANCE.and.&
  &  abs(a(3,3)-4d0)<TOLERANCE)then
    call showPass('SerialPack store ListDVect')
  else
    call showFail('SerialPack store ListDVect')
  end if
